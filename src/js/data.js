    ...
    if (!hasSpender) {
      ...
      spendersOverview.forEach((spender, index) => {
        if (spender.key === bill.spender.key) {
          ...
          bill.isPaid
            ? (spender.amountPaid += bill.amount)
            : (spender.amountDue += bill.amount);
        }
      });
    }
    ...
    if (!hasCategory) {
      ...
      categoriesOverview.forEach((category, index) => {
        if (category.key === bill.category.key) {
          ...
          bill.isPaid
            ? (category.amountPaid += bill.amount)
            : (category.amountDue += bill.amount);
        }
      });
    }
  });
